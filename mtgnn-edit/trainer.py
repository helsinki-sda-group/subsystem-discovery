...
class Trainer():
    ...
    def train(self, input, real_val, idx=None):
        ...
        num_devices = max(len(self.model.device_ids), 1)
        output = self.model(input, idx.unsqueeze(0).repeat(num_devices, 1))
        output = output.transpose(1,3)
        real = torch.unsqueeze(real_val,dim=1)
        predict = self.scaler.inverse_transform(output, idx)
    ...
    def eval(self, input, real_val):
        ...
        if input.device != real_val.device:
            print("Tensors are on different devices, moving real val to input's device.")
            real_val = real_val.to(input.device)

        #num_devices = len(self.model.device_ids)
        num_devices = max(len(self.model.device_ids), 1)
        idxs = torch.arange(input.shape[-2]).unsqueeze(0).repeat(num_devices, 1) #.to(self.model.device)
        output = self.model(input, idxs)
        ...
        # normalized verrsion
        #predict = self.scaler.inverse_transform(output, torch.arange(output.shape[2]))
        # real as is
        ...
        return loss.item(),mape,rmse,mse
